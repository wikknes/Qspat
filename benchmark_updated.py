...
from src.real_vqe import RealSpatialVQE
from src.real_qaoa import RealSpatialQAOA
...
class Benchmark:
    ...
    def __init__(self, data_path=DEFAULT_DATA_PATH, target_gene=DEFAULT_TARGET_GENE,
                 max_spots=DEFAULT_MAX_SPOTS, quantum_backend='statevector_simulator',
                 shots=1024):
        """
        ...
        Args:
            ...
            quantum_backend: Qiskit backend to use for quantum algorithms
            shots: Number of shots for measurement-based backends
        """
    ...
    def run_qspat_vqe(self):
        ...
        # Create VQE solver with real implementation
    ...
    def run_qspat_qaoa(self, threshold=0.5):
        ...
        # Create QAOA solver with real implementation
    ...
    def generate_markdown_report(self):
        ...
        report = [
            ...
            f"- Quantum Backend: {self.quantum_backend}",
        ...
        if max_available and region_available:
            ...
            # VQE analysis
            if 'qspat_vqe' in self.results:
                quantum_pros.append("- VQE represents the state space efficiently with logarithmic qubit requirement")
                quantum_pros.append("- VQE enables simultaneous exploration of multiple potential maxima")
                quantum_pros.append("- VQE's probabilistic approach provides insights into expression distribution patterns")
            
            # QAOA analysis
            if 'qspat_qaoa' in self.results:
                quantum_pros.append("- QAOA directly optimizes for both expression level and spatial coherence together")
                quantum_pros.append("- QAOA can represent complex spatial relationships that may be hard to encode classically")
                quantum_pros.append("- QAOA identified regions with good balance of expression and spatial cohesion")
            ...
            classical_pros = [
                "- Classical methods offer excellent performance for current dataset sizes",
                "- Direct optimization approaches are very effective for finding expression maxima",
                "- Classical clustering methods provide an excellent balance of speed and region quality",
                "- Classical methods have a mature, optimized software ecosystem"
            ]
            ...
            future_work = [
                ...
                "- Optimize quantum circuit design for improved convergence",
                "- Explore quantum advantage points with more complex objective functions",
                "- Implement multi-gene analysis to find co-expression patterns",
                "- Investigate hardware-specific optimizations for quantum backends"
            ]
...
def main():
    ...
    parser.add_argument('--quantum_backend', default='statevector_simulator', 
                      help='Qiskit backend to use (default: statevector_simulator)')
